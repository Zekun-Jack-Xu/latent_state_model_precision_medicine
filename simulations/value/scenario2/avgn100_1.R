...
colnames(bmean) <- c("obs","stolin","stoquad","stomdplin","stomdpquad",
                     "detlin","detquad","detmdplin","detmdpquad", "opt")
...
while(b<=nboot){
  ...
  utilfun0 <- function(obj){
      ...
      obj$lastobj$delta[1],obj$lastobj$delta[2],obj$lastobj$delta[3],
      obj$lastobj$delta[4],obj$lastobj$delta[5])
      ...
      #u <-  0.5*p5last + 0.3*ifelse(trt==3,1,0) - abs(x1) - abs(x3) #* abs(x3)
      u <- (p1+p4)*ifelse(trt==1,1,-1) + (p2+p3)*ifelse(trt==2,1,-1) +
      p5*ifelse(trt==3,1,-1)
      return(u)
  }
  ...
  usumdis <- try(sapply(1:nsubj,function(k){
    mean(ulist1[[k]],na.rm=TRUE) }),silent=TRUE)
  ...
  for(j in 1:nsubj){
    ...
    else if(nrow(xlist[[j]])<=2) nullid <- c(nullid,j)
  }
  ...
  #clean up the last one since no k+1 for recursive estimation
  ...
  #######################################
  augdata1 <- transform(augdata, x1=x1/100,x2=x2/100,x3=x3/100,
                        xnext1=xnext1/100,xnext2=xnext2/100,xnext3=xnext3/100)
  nsubjrow <- length(trimulist)
  
  
  valuename1 <- c("p1t", "p2t","p3t","p4t" )
  ...
  ktrt=3
  augvalueparm <- rep(0, length(valuename1)+2)
  policyparm <- rep(0, (length(valuename1)+1)*(ktrt-1))
  
  
  print("fit pomdp lin")
  plin <- try(fit.sto.avg(augvalueparm, policyparm,
  ...
  trtfun2 <- function(obj){
      ...
      coef1v3 <- plin$policyparm[1:5]
      coef2v3 <- plin$policyparm[6:10]
      lp1v3 <- coef1v3 %*% vec
      lp2v3 <- coef2v3 %*% vec
      p1 <- exp(lp1v3) / (1 + exp(lp1v3) + exp(lp2v3))
      p2 <- exp(lp2v3) / (1 + exp(lp1v3) + exp(lp2v3))
      ...
      if(class(temp)=="try-error") next
        ...
        temp <- lapply(1:length(temp),function(k) temp[[k]][-1])
          this2list <- try(sapply(1:nsubj,function(k){
              mean(temp[[k]], na.rm=TRUE) }),silent=TRUE)
          if(class(this2list)=="try-error") next
          this2[j] <- mean(this2list, na.rm=TRUE)
          j <- j+1
      }
  }
 if(mean(this2)<obsmean) next
  ###################################################
  #quadratic
  valuename1 <- c("p1t", "p2t","p3t","p4t", 
                  "p1tq","p2tq","p3tq","p4tq" )
  valuename2 <- c("p1tp1", "p2tp1","p3tp1","p4tp1" ,
                  "p1tp1q", "p2tp1q","p3tp1q","p4tp1q")
  policyname <- c("p1t", "p2t","p3t","p4t" )
  
  augvalueparm <- rep(0, length(valuename1)+2)
  
  print("fit pomdp quad")
  pquad <- try(fit.sto.avg(augvalueparm, policyparm,
                       nsubj, augdata1, valuename1, valuename2, policyname,
                       ppsmat, ktrt,
                       tausq=tausq, kappa=kappa, type=type,
                       tuning1=tuning1, tuning2=tuning2, print=FALSE),
  ...
  trtfun3 <- function(obj){
      vec <- c(1,
      obj$delta[1],obj$delta[2],obj$delta[3],obj$delta[4])
      coef1v3 <- pquad$policyparm[1:5]
      coef2v3 <- pquad$policyparm[6:10]
      lp1v3 <- coef1v3 %*% vec
      lp2v3 <- coef2v3 %*% vec
      p1 <- exp(lp1v3) / (1 + exp(lp1v3) + exp(lp2v3))
      p2 <- exp(lp2v3) / (1 + exp(lp1v3) + exp(lp2v3))
 ...
 if(mean(this3)<obsmean) next
  ...
  print("fit mdp lin")
  plin_mdp <- try(fit.sto.avg(augvalueparm, policyparm,
  ...
  valueparm <- rep(0, length(valuename1)+2)
  print("fit mdp quad")
  pquad_mdp <- try(fit.sto.avg(augvalueparm, policyparm,
                           nsubj, augdata, valuename1, valuename2, policyname,
                           ppsmat, ktrt,
                           tausq=tausq, kappa=kappa, type=type,
                           tuning1=tuning1, tuning2=tuning2, print=FALSE),
  ...
  while(j<=nsims){
    ...
    if(class(temp)=="try-error") next
      ...
      this6list <- try(sapply(1:nsubj,function(k){
        mean(temp[[k]], na.rm=TRUE) }),silent=TRUE)
      if(class(this6list)=="try-error") next
  ...
  trtfun10 <- function(obj){
    vec <- c(1, obj$x[1],obj$x[2],obj$x[3], #2:4
             obj$delta[1],obj$delta[2],obj$delta[3],obj$delta[4],obj$delta[5]) #5:9
    ...
    decision <- which.max(c(p1,p2,p3,p4,p5))
    if(decision==5){
      trt <- 3
      trtcontrast <- c(0,0)
    }else if(decision%in%c(2,3)){
      trt <- 2
      trtcontrast <- c(0,1)
    } else{
      trt <- 1
...
save(list=c("bmean"), file="avgn100_1.RData")
