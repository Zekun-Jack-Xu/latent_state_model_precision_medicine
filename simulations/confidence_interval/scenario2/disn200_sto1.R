library(rarhsmm, lib.loc="~/R-Library")
library(nnet)
...
while(b<=nboot){
  ...
  disfactor,ppsmat,ktrt), silent=TRUE)
  if(class(value_est)=="try-error") next
  ...
  print("fit pomdp lin")
  popt <- try(fit.sto.dis(valueparm, policyparm,
                          nsubj, augdata1, valuename1, valuename2, policyname,
                          disfactor=disfactor,ppsmat=ppsmat,ktrt=ktrt,
                          tausq=tausq, kappa=kappa, type=type,
                          tuning1=tuning1, tuning2=tuning2, print=FALSE),
  ...
  while(jj <= 50){ 
    ...
    meanselist <- try(get_meanse_dis(newparm, augdata1, ppsmat, nsubj,
                                     ...
                                     policyname, ktrt=3, discount=0.9), silent=TRUE)
...
save(list=c("covervec","widthvec"), file="disn200_sto1.RData")
