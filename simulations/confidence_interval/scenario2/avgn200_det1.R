...
while(b<=nboot){
  ...
  augvalueparm <- c(0,-1.5,-1.5,-1.5,-1.5,0,0,0,0)
  policyparm <- c(10,0.5,-0.5,-0.5,0.5,0,0,0,10,-0.5,0.5,0.5,-0.5,0,0,0)
  
  print("fit pomdp lin")
  plin <- try(fit.det.avg(augvalueparm, policyparm,
  ...
  tuning1=1e-8
  tuning2=1e-4
  popt <- try(fit.det.avg(augvalueparm, policyparm,
                          nsubj, augdata1, valuename1, valuename2, policyname,
                          ppsmat, ktrt,
                          tausq=tausq, kappa=kappa, type=type,
                          tuning1=tuning1, tuning2=tuning2, print=FALSE),
  ...
  time <- proc.time()
  gd <- numDeriv::grad(value_avg, plin$policyparm, augvalueparm=plin$augvalueparm, 
                       nsubj=nsubj, data=augdata1,
                       valuename1=valuename1, valuename2=valuename2,
                       policyname=policyname, ppsmat=ppsmat, ktrt=3)
  proc.time() - time
  time <- proc.time()
  hs <- numDeriv::hessian(value_avg, plin$policyparm, augvalueparm=plin$augvalueparm, 
                          nsubj=nsubj, data=augdata1,
                          valuename1=valuename1, valuename2=valuename2,
                          policyname=policyname,  ppsmat=ppsmat, ktrt=3)
  proc.time() - time
  ...
  while(jj <= 30){ 
    ...
    meanselist <- try(get_meanse_avg(newparm, augdata1, ppsmat, nsubj,
                                 valuename1, valuename2,
                                 policyname, ktrt=3, type="stochastic"), silent=TRUE)
                                 
    if(class(meanselist)=="try-error") next
    if(is.na(meanselist$value_se) | meanselist$value_se>50) next
    value_est <- plin$augvalueparm[length(plin$augvalueparm)]
...
save(list=c("covervec","widthvec"), file="avgn200_det1.RData")
